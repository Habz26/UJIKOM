...
@section('content')
    <div class="container-fluid">
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <form method="POST" action="{{ route('loans.return', $loan) }}"
                                                class="d-inline" onsubmit="return confirm('Kembalikan sekarang?')">
                                                @csrf @method('PATCH')
                                            ...
                                            <button class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                                data-bs-target="#conditionModal{{ $loan->id }}">
                                                <i class="bi bi-clipboard-check"></i> Kondisi
                                            </button>
                                        </div>
                                    </td>
                                </tr>
        ...
        @foreach ($loans as $loan)
            <div class="modal fade" id="conditionModal{{ $loan->id }}" tabindex="-1">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-warning text-dark">
                            <h5>Verifikasi Kondisi - {{ $loan->book->title }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <form action="{{ route('loans.update-condition', $loan) }}" method="POST">
                            @csrf @method('PATCH')
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="damage_note" class="form-control" rows="3">{{ $loan->damage_note ?? '' }}</textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                <button type="submit" class="btn btn-warning">Simpan & Kembalikan</button>
                            </div>
                        </form>
                    </div>
                </div>
