            ...
            <div class="mb-6 p-6 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl shadow-xl">
                ...
                @php
                    ...
                    if (($stats['overdue_count'] ?? 0) > 0) {
                        $statusParts[] =
                            '<strong class="text-red-600">(' . ($stats['overdue_count'] ?? 0) . ' terlambat)</strong>';
                    }
                ...
                @if (!empty($allText))
                    <div class="text-lg text-gray-700">
                        ...
                        <a href="{{ route('loans.active') }}"
                            class="text-green-600 hover:text-green-800 font-semibold underline ms-2">
        ...
        <div class="row g-4 mb-5">
            <div class="col-md-6 col-lg-3">
                <a href="{{ route('books.index') }}"
                    class="card h-100 text-decoration-none shadow-sm border-0 hover-shadow-xl transition-all">
                                    ...
                                    @forelse($activeLoans as $loan)
                                        <tr class="@if ($loan->return_date < now()) table-danger @endif">
                                            <td>
                                                ...
                                                <br><small class="text-muted">Kategori:
                                                    {{ $loan->book->kategori?->name ?? 'N/A' }}</small>
                                            </td>
                                            ...
                                            <td>
                                                @if ($loan->return_date > now())
                                                    <span
                                                        class="text-success fw-bold">{{ round($loan->return_date->diffInDays(now())) }}
                                                        hari</span>
                                                @else
                                                    <span
                                                        class="text-danger fw-bold">{{ round(now()->diffInDays($loan->return_date)) }}
                                                        hari terlambat</span>
                                                @endif
                                            </td>
                                        </tr>
